    ...
    <div className="glass-panel h-full">
      ...
      <div className="card-content pb-0 max-h-[600px] overflow-y-auto scrollbar-hide">
        <div className="space-y-6">
          <section>
            <h4 className="text-lg font-medium mb-4 text-slate-200">Common Skin Lesion Types</h4>
            <div className="space-y-4">
              {skinLesions.map((lesion, index) => (
                ...
                >
                  ...
                  />
                  <div>
                    <h5 className="font-medium text-slate-200">{lesion.name}</h5>
                    <p className="text-sm text-slate-400">{lesion.description}</p>
                  </div>
                </motion.div>
              ))}
            </div>
          ...
          <section>
            <h4 className="text-lg font-medium mb-4 text-slate-200">Frequently Asked Questions</h4>
              ...
              {faqItems.map((item, index) => (
                <div key={index} className="border border-slate-700 rounded-lg overflow-hidden">
                  ...
                  >
                    ...
                    {expandedFaq === index ? (
                      <ChevronUp className="w-5 h-5 flex-shrink-0 text-cyan-500" strokeWidth={1.5} />
                    ) : (
          ...
          <div className="bg-cyan-500/10 border border-cyan-500/30 rounded-lg p-4 flex items-start">
            <Info className="w-5 h-5 text-cyan-500 mr-3 mt-0.5 flex-shrink-0" strokeWidth={1.5} />
            <div>
              <h5 className="font-medium text-cyan-400">Early Detection Matters</h5>
      ...
      <div className="card-footer flex-col items-start border-t border-slate-700 mt-6">
        <h4 className="font-medium mt-2 text-slate-200">Need Professional Advice?</h4>
        <p className="text-sm text-slate-400 mb-3">
          This tool is not a substitute for professional medical advice. Always consult with a dermatologist.
        </p>
        <a
          href="#"
          className="btn btn-outline btn-sm w-full text-cyan-500 border-cyan-500 hover:bg-cyan-500/10"
        >
